...
<head>
    ...
    <title>Pengeditan Kosa Kata</title>
</head>
<body>
    ...
    <div class="block text-center my-[50px] bg-[#80B3FF] w-[35%] py-10 rounded-xl mx-auto">
        <form action="/Pengeditan" method="POST" class="inline-block" enctype="multipart/form-data">
            ...
            <div class="relative">
                <input type="file" name="format" class="block w-[400px] my-[10px] bg-white px-3 py-1 rounded-lg lg:pr-5 lg:py-2">
                <label for="format" class="absolute text-sm text-gray-500 transform -translate-y-4 scale-75 top-4 z-10 origin-[0] left-2.5">Format Penulisan</label>
                    {{-- @error('format')
                        <div class="text-red-200 text-left">{{ $message }}</div>
                    @enderror --}}
            </div>
