...
<body class="font-[Poppins]">
    <div class="relative">
        <div class="text-3xl py-10 pl-5 bg-[#4E6C50] md:pl-10">
            <a href="/pesan1-admin" class="text-white">< {{ $user }}</a>
        </div>
        <div>
            @foreach ($chat as $c)
                @if (empty($c->balas))
            ...
            <div class="fixed bottom-2 left-5 right-[3%] md:left-10">
                <form action="/chatAdmin/{{ $id }}" method="POST" class="flex">
                    @csrf
                    <input type="text" name="balas" placeholder="Masukkan pesan anda..." class="w-[1500px] py-2 pl-5 bg-[#4E6C50] text-white rounded-md placeholder:text-white">
